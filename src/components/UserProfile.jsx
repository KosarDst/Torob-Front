...
const UserProfile = () => {
  ...
  useEffect(() => {
    ...
    if (!hasData) {
      ...
      axios
        .get("http://lcoalhost:8000/api/v0/post/page/post1", currUser)
        .then((info) => {
          setUserProd(info.data.data);
        })
        .catch((err) => err);
      /* get shops */
      axios.get("http://lcoalhost:8000/api/v0/shop", currUser).then((info) => {
        setShops(info.data.data);
      });
    }
    return () => {
      hasData = true;
  ...
  const handleSignout = () => {
    const currUser = localStorage.getItem("userDetail");
    dispatch(signout());
    axios.get("http://lcoalhost:8000/api/v0/auth/signout", currUser);
    navigate("/");
  };
  ...
  const handleAddShop = (e) => {
    const { name } = e.target;
    const currUser = localStorage.getItem("userDetail");
    axios.post("http://lcoalhost:8000/api/v0/shop", currUser).then((info) => {
      setShops(info.data.data);
    });
    setShops((prevValue) => {
